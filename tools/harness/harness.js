...
`;
  ...
  var scenarios = [
    ...
    { name: "delete completed tasks", async run({ mock: mock2, F, check }) {
      const button = q("#clear-completed-btn");
      const other = F.Work;
      const original = "# Todo\n\n- [x] Done parent\n  - [x] Done child\n    - [ ] Keep grandchild\n  - [ ] Keep sibling\n\n---\nSome notes\n- [ ] Keep root\n";
      F.Todo = original;
      mock2.emit("todo:snapshot", mock2.snapshot());
    ...
    { name: "delete completed tasks while markdown save is pending", async run({ mock: mock2, F, check }) {
      const other = F.Work;
      key("e", { metaKey: true });
      ...
      try {
        ...
        check("other tab unchanged and button disabled", F.Work === other && q("#clear-completed-btn").disabled);
      } finally {
        core.invoke = invoke;
      }
    } },
