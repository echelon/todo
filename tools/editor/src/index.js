...
// with vim emulation, exposed as the global `TodoEditor`.
 ...
 * Create an editor.
 * @param {Object} o
 * @param {HTMLElement} o.parent   mount point
 * @param {string} o.doc           initial text
 * @param {boolean} o.vim          start with vim keys
 * @param {(text:string)=>void} o.onChange
 * @param {()=>void} [o.onSave]    `:w` / Ctrl-S
 * @param {()=>void} [o.onQuit]    `:q`
 */
export function create(o) {
  ...
  return {
    ...
    setVim(on) { view.dispatch({ effects: vimComp.reconfigure(on ? vim({ status: true }) : []) }); },
    /** Current vim mode ('normal' | 'insert' | 'visual' | …) or null when vim is off. */
    mode() { const cm = getCM(view); return cm?.state?.vim ? (cm.state.vim.insertMode ? 'insert' : cm.state.vim.mode || 'normal') : null; },
